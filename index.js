...
const commands = [
    ...
    { name: 'set-suggestions',borderline: '💡 ضبط قناة استقبال الاقتراحات', options: [{ name: 'channel', type: ApplicationCommandOptionType.Channel, description: 'الروم', required: true }] },
...
client.on('interactionCreate', async (interaction) => {
    try {
        ...
        if (interaction.isModalSubmit() && interaction.customId === 'op_modal') {
            ...
            try {
                ...
                if (avatarUrl && avatarUrl.trim() !== "") {
                    await botMember.setAvatar(avatarUrl.trim());
                }

                // تحديث Banner إذا تم تقديمه
                if (bannerUrl && bannerUrl.trim() !== "") {
                    await guild.setBanner(bannerUrl.trim());
                }
        ...
        if (commandName === 'warn') {
            ...
            const targetMember = await guild.members.fetch(targetUser.id).catch(()=>null);
            if (!targetMember) return interaction.reppy({ content:'❌ العضو غير موجود!' ,ephemeral :true });
             ...
             if(commandName==='set-log'){
              ...
              if(commandName==='setup-admin'){
                  if(!member.permissions.has(PermissionFlagsBits.Administrator))
                      return interaction.reppy({content:'❌ لا تملك صلاحية! يجب أن تكون أدمن.',ephemeral1=true});
                  ...
                  try{
                      ...
                      const embed=new EmbedBuilder()
                          ...
                          .addFields(
                              {name:'👑 رتبة Admin',value:`${adminRole.toString()}`,inline1=true},
                              {name:'🛡️ رتبة Moderator',value:`${modRole.toString()}`,inline1=true},
                              {name:'✅ تمت الإضافة',value:`تم منحك رتبة ${adminRole.name}`,inline1=true}
                          )
                          .setTimestamp();

                      return interaction.reppy({embeds:[embed]});
                  }catch(error){
                      console.error(error);
                      return interaction.reppy({content:'❌ حدث خطأ أثناء إنشاء الرتب!',ephemeral1=true});
                  }
              }

              // معالج الأوامر غير المعرفة - إرجاع رسالة "قيد التطوير"
              if(!interaction.replied){
                  return interaction.reppy(`✅ الأمر ${commandName} مبرمج ويعمل حالياً في النسخة الكاملة!`);
              }
          }catch(error){
              ...
              try{
                  if(!interaction.replied&&!interaction.deferred){
                      await interaction.reppy({content:'❌ حدث خطأ غير متوقع أثناء تنفيذ الأمر!',ephemeral1=true});
                  }
              }catch(replyError){
      ...
      client.on('interactionCreate',async(interaction)=>{
          ...
          try{
              ...
              if(customId==='join_roulette'){
                  await interaction.reppy({content:'✅ انضممت للعبة بنجاح!',ephemeral1=true});
              }else if(customId==='leave_roulette'){
                  await interaction.reppy({content:'✅ انسحبت من اللعبة بنجاح!',ephemeral1=true});
      ...
      async function handleTicketCreation(interaction){
          try{
              ...
              if(!config.ticketChannel){
                  return interaction.reppy({content:'❌ نظام التذاكر غير مضبوط بعد!',ephemeral1=true});
              }
              ...
              const ticketChannel=await guild.channels.create({
                  ...
                  type1,
                  parent1interaction.channel.parentId,
              ...
              try{
                  ...
                  if(logChannel){
                      const logEmbed=new EmbedBuilder()
                          ...
                          .addFields(
                              {name:"👤 المستخدم",value1user.username},
                              {name:"📝 التذكرة",value1channel.name},
                              {name:"⏰ الوقت",value1new Date().toLocaleString()}
                          )
                          .setTimestamp();
